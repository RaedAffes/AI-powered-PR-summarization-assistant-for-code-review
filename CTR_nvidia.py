...
from utils import ctr_prompt, ctr_prompt_summary, ct_formatter, remove_diffs, count_matching_elements, load_checkpoint, save_checkpoint
...
def main():
    ...
    pbar = tqdm(total=len(data), desc=f"CTR | {model_name}")
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
        ...
        for ex_idx in remaining:
            prompt_permutations, correct_answers, combinations = prompt_combinations(data[ex_idx], language_type or "code", use_summary)
            in_flight[ex_idx] = {
        ...
        for fut in as_completed(pending):
            ...
            if all(a is not None for a in in_flight[ex_idx]["answers"]):
                ...
                pbar.set_description(f"CTR | {model_name} | inv={inv}/{len(partial)}")
                pbar.update(1)
